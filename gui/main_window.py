...
class AccountBookGUI:
    ...
    def edit_entry(self):
        ...
        # 密码验证
        password = show_password(self.root, "请输入密码以验证修改操作：")
        if not password:
            return
        
        # 验证密码（使用当前登录用户的密码）
        user = self.user_manager.authenticate(self.current_user.username, password)
        if not user:
            messagebox.showerror("错误", "密码不正确，无法修改账目")
            return
    ...
    def delete_entry(self):
        ...
        # 密码验证
        password = show_password(self.root, "请输入密码以验证删除操作：")
        if not password:
            return
        ...
        user = self.user_manager.authenticate(self.current_user.username, password)
        if not user:
            messagebox.showerror("错误", "密码不正确，无法删除账目")
            return
    ...
    def import_csv(self):
        """导入CSV数据"""
        # 密码验证
        password = show_password(self.root, "请输入密码以验证导入操作：")
        if not password:
            return
        ...
        user = self.user_manager.authenticate(self.current_user.username, password)
        if not user:
            messagebox.showerror("错误", "密码不正确，无法导入账目")
            return
